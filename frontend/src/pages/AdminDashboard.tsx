...
const emptyForm: VehicleFormData = { make: "", model: "", category: "", price: "", quantity: "" };
...
const VehicleModal = ({ open, title, form, busy, onClose, onChange, onSubmit, submitLabel }: VehicleModalProps) => {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-lg overflow-hidden rounded-lg border border-asphalt-700 bg-asphalt-800">
        <div className="flex items-center justify-between border-b border-asphalt-700 px-6 py-5">
          <h2 className="font-display text-xl font-bold uppercase tracking-wide text-chrome-100">{title}</h2>
          <button onClick={onClose} className="focus-ring rounded-md p-2 text-chrome-500 hover:text-chrome-100">
        ...
        <div className="grid gap-4 p-6 sm:grid-cols-2">
          {(
            [
              ...
              { field: "category", label: "Category", placeholder: "Sedan" },
              { field: "price", label: "Price ($)", placeholder: "25000", type: "number" },
            ] as const
          ).map(({ field, label, placeholder, type }) => (
            <div key={field}>
              <label className="mb-1 block text-xs font-medium uppercase tracking-wide text-chrome-500">{label}</label>
              <input
                type={type ?? "text"}
                value={form[field]}
          ...
          <div className="sm:col-span-2">
            <label className="mb-1 block text-xs font-medium uppercase tracking-wide text-chrome-500">Quantity</label>
            <input
              type="number"
              min="0"
              value={form.quantity}
...
const AdminDashboard = () => {
  ...
  const updateField = (field: keyof VehicleFormData, val: string) => setForm((f) => ({ ...f, [field]: val }));

  const filteredTableVehicles = useMemo(() => {
    const q = searchTable.toLowerCase();
    return vehicles.filter(
      (v) => v.make.toLowerCase().includes(q) || v.model.toLowerCase().includes(q) || v.category.toLowerCase().includes(q),
    );
  ...
  const handleDelete = async (v: Vehicle) => {
    if (!window.confirm(`Delete ${v.make} ${v.model}? This cannot be undone.`)) return;
    try {
    ...
    <div className="min-h-screen bg-asphalt-900">
      ...
      <main className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <span className="font-mono text-xs font-semibold uppercase tracking-widest text-ignition">Admin Console</span>
            ...
            <p className="mt-1 text-chrome-500">
              Logged in as <strong className="text-chrome-300">{user?.email}</strong>
            </p>
          </div>
          <button
            onClick={openCreate}
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          {[
            { label: "Total Vehicles", value: stats.total, color: "text-chrome-100" },
            { label: "Out of Stock", value: stats.outOfStock, color: "text-red-400" },
            { label: "Low Stock (≤3)", value: stats.lowStock, color: "text-ignition" },
            { label: "Categories", value: stats.categories, color: "text-emerald-400" },
          ].map(({ label, value, color }) => (
            <div key={label} className="rounded-lg border border-asphalt-700 bg-asphalt-800 p-4">
        ...
        <div className="overflow-hidden rounded-lg border border-asphalt-700 bg-asphalt-800">
          ...
          <div className="overflow-x-auto">
            {loading ? (
              <div className="flex items-center justify-center py-20 text-sm text-chrome-500">Loading inventory…</div>
              ...
              <table className="w-full min-w-[700px] text-sm">
                ...
                <tbody className="divide-y divide-asphalt-700">
                  {filteredTableVehicles.map((v) => (
                    <tr key={v.id} className="transition hover:bg-asphalt-900/60">
                      <td className="px-5 py-4">
                        <p className="font-semibold text-chrome-100">
                          {v.make} {v.model}
                        </p>
                        <p className="font-mono text-xs text-chrome-500">{v.id.slice(0, 8)}…</p>
                      </td>
                      <td className="px-5 py-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-sm rounded-lg border border-asphalt-700 bg-asphalt-800">
            <div className="border-b border-asphalt-700 px-6 py-5">
              <h2 className="font-display text-xl font-bold uppercase tracking-wide text-chrome-100">Restock Vehicle</h2>
              <p className="mt-1 text-sm text-chrome-500">
                {selectedVehicle?.make} {selectedVehicle?.model} — currently{" "}
                <strong className="text-chrome-300">{selectedVehicle?.quantity}</strong> in stock
              </p>
            </div>
            <div className="p-6">
              <label className="mb-1.5 block text-sm font-medium text-chrome-300">Amount to add</label>
              <input
                type="number"
                min="1"
                value={restockAmount}
